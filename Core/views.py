...
def main(request):
    ...
    new_products = Product.objects.filter(available=True).order_by('-created_at').all()[:25]
    hit_products = Product.objects.filter(available=True).order_by('-rating').all()[:25]
